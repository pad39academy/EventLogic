...
export class UploadService {
  ...
  // ==============================================================
  // COORDINATED UPLOAD METHODS WITH OPERATION LOCKING
  // ==============================================================

  /**
   * Coordinated hotel inventory upload with operation locking
   */
  static async uploadHotelInventoryWithCoordination(
    content: string,
    userId: string,
    sessionId: string
  ): Promise<CoordinatedUploadResult> {
    
    console.log(`📤 Starting coordinated hotel upload for user ${userId}`);
    
    // Step 1: Try to acquire operation lock
    const lockResult = await OperationLockService.acquireOperationLock(
      'hotel_upload', 
      userId, 
      sessionId,
      'global',
      45 // 45 minute timeout for hotel uploads
    );
    
    if (!lockResult.granted) {
      console.log(`⚠️ Hotel upload blocked: ${lockResult.reason}`);
      
      // Queue the operation for later processing
      const queueEntry = await OperationQueueService.queueOperation(
        'hotel_upload',
        userId,
        sessionId,
        { content }, // Store the content for later processing
        1 // High priority
      );
      ...
      return {
        success: false,
        created: 0,
        errors: [lockResult.reason || 'Operation blocked by conflicting operations'],
        warnings: [`Your upload has been queued at position ${queueEntry.position}`],
        canProceed: false,
        queueInfo: {
    ...
    console.log(`✅ Lock acquired for hotel upload, proceeding...`);
    let uploadResult: UploadResult;
    
    try {
      // Use the batch optimized version for better performance
      uploadResult = await this.uploadHotelInventoryBatch(content);
      
      console.log(`🎉 Hotel upload completed: ${uploadResult.created} records created`);
      
    } catch (error) {
      console.error('❌ Hotel upload failed:', error);
      uploadResult = {
        success: false,
        created: 0,
      ...
      const released = await OperationLockService.releaseOperationLock(lockResult.lock!.id, userId);
      if (released) {
        console.log(`🔓 Hotel upload lock released`);
        
        // Process any queued operations that can now proceed
        await OperationQueueService.processQueueForOperationType('hotel_upload');
      }
    }
  ...
  /**
   * Coordinated coaches/officials upload with operation locking
   */
  static async uploadCoachesOfficialsWithCoordination(
    content: string,
    userId: string,
    sessionId: string
  ): Promise<CoordinatedUploadResult> {
    
    console.log(`📤 Starting coordinated coaches/officials upload for user ${userId}`);
    
    // Step 1: Try to acquire operation lock
    const lockResult = await OperationLockService.acquireOperationLock(
      'coaches_officials_batch', 
      userId, 
      sessionId,
      'global',
    ...
    if (!lockResult.granted) {
      console.log(`⚠️ Coaches/officials upload blocked: ${lockResult.reason}`);
      ...
      const queueEntry = await OperationQueueService.queueOperation(
        ...
        sessionId,
        { content },
        4 // Lower priority than hotels but higher than players
      );
    ...
    // Step 2: Proceed with the upload
    console.log(`✅ Lock acquired for coaches/officials upload, proceeding...`);
    let uploadResult: UploadResult;
    
    try {
      uploadResult = await this.uploadCoachesOfficials(content);
      console.log(`🎉 Coaches/officials upload completed: ${uploadResult.created} records created`);
      
    } catch (error) {
      console.error('❌ Coaches/officials upload failed:', error);
      uploadResult = {
        success: false,
        created: 0,
        errors: [`Upload failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
      ...
      const released = await OperationLockService.releaseOperationLock(lockResult.lock!.id, userId);
      if (released) {
        console.log(`🔓 Coaches/officials upload lock released`);
        await OperationQueueService.processQueueForOperationType('coaches_officials_batch');
      }
    }
  ...
  /**
   * Coordinated player upload with operation locking
   */
  static async uploadPlayersWithCoordination(
    content: string,
    userId: string,
    sessionId: string
  ): Promise<CoordinatedUploadResult> {
    
    console.log(`📤 Starting coordinated player upload for user ${userId}`);
    ...
    if (!lockResult.granted) {
      console.log(`⚠️ Player upload blocked: ${lockResult.reason}`);
    ...
    // Step 2: Proceed with the upload
    console.log(`✅ Lock acquired for player upload, proceeding...`);
    let uploadResult: UploadResult;
    
    try {
      // Use batch optimized version
      uploadResult = await this.uploadPlayersBatch(content);
      console.log(`🎉 Player upload completed: ${uploadResult.created} records created`);
      ...
      const released = await OperationLockService.releaseOperationLock(lockResult.lock!.id, userId);
      if (released) {
        console.log(`🔓 Player upload lock released`);
        await OperationQueueService.processQueueForOperationType('player_upload');
      }
    }
